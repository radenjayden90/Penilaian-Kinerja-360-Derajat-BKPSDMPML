...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <form method="GET" action="{{ route('master.positions.index') }}" class="row g-2" x-data @submit.prevent="Livewire.navigate($el.action + '?' + new URLSearchParams(new FormData($el)).toString())">
            ...
            <div class="col-12 col-md-3 d-flex gap-2">
                <select name="status" class="form-select bg-light" onchange="this.form.requestSubmit()" style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; padding-right: 2.5rem;">
                    <option value="">-- Semua Status --</option>
                    <option value="1" {{ request('status') === '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ request('status') === '0' ? 'selected' : '' }}>Nonaktif</option>
                </select>
                @if(request('search') || request('department_id') || request('status') !== null)
                    <a href="{{ route('master.positions.index') }}" class="btn btn-outline-secondary" title="Reset Filter" wire:navigate>
