    ...
    <div class="p-4 sm:p-6">
        <form action="{{ route('master.employees.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-form.input name="nip" label="NIP" value="{{ old('nip') }}" error="{{ $errors->first('nip') }}" required />
                <x-form.input name="name" label="Nama Lengkap" value="{{ old('name') }}" error="{{ $errors->first('name') }}" required />
                <x-form.input name="email" label="Email" type="email" value="{{ old('email') }}" error="{{ $errors->first('email') }}" required />
                <x-form.input name="password" label="Password (Opsional, default: NIP)" type="password" error="{{ $errors->first('password') }}" />
                <x-form.input name="phone" label="Nomor HP" value="{{ old('phone') }}" error="{{ $errors->first('phone') }}" />
